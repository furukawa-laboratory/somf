    ...
    def fit(self, nb_epoch=200):
        ...
        if self.hantei_X == 1:
            for epoch in tqdm(np.arange(nb_epoch)):
                ...
                H1 = np.exp(-distance1 / (2 * pow(sigma1, 2))).T  # かっこに気を付ける
                ...
                H2 = np.exp(-distance2 / (2 * pow(sigma2, 2))).T  # かっこに気を付ける
                # G2 = np.sum(H2, axis=1)  # Gは行ごとの和をとったベクトル
                # R2 = (H2.T / G2).T  # 行列の計算なので.Tで転置を行う

                G = np.einsum("ik,jl,ij->kl", H1, H2, self.gamma)

                # ２次モデルの決定
                self.Y = np.einsum('ik,jl,ij,ijd->kld', H1, H2, self.gamma, self.X)/G[:,:,None]

                # 勝者決定
                Dist = self.gamma[:, :, None, None, None] * np.square(self.X[:, :, None, None, :] - self.Y[None, None, :, :, :])
                self.k_star1 = np.argmin(np.einsum("jl,ijklm->ik", H2, Dist), axis=1)
                self.k_star2 = np.argmin(np.einsum("ik,ijklm->jl", H1, Dist), axis=1)
