...
def anime_reference_vector_3d(X,allY):
    ...
    D = allY.shape[2]
    T = allY.shape[0]
    resolution = np.int(np.sqrt(K))
    re_allY=allY.reshape((T, resolution, resolution, D))
    ax.scatter(X[:,0],X[:,1],X[:,2], color='g')
    wframe = ax.plot_wireframe(re_allY[0,:,:,0],re_allY[0,:,:,1],re_allY[0,:,:,2],color='b')
