...
class TSOM2_Umatrix:
    def __init__(self, z1=None, z2=None, x=None, sigma1=0.2, sigma2=0.2, resolution=100, labels1=None, labels2=None,
                 fig_size=[15, 15], cmap_type='jet'):
        # set input
        self.X = x
        I = self.X.shape[0]
        J = self.X.shape[1]
    ...
    def draw_umatrix(self):
        ...
        if self.Zeta2[:, 0].min() != self.Zeta2[:, 0].max():
            ...
            self.Map2.imshow(U_matrix_val2.T, interpolation='spline36',
                             extent=[self.Zeta2[:, 1].min(), self.Zeta2[:, 1].max(), -1.3, -0.7],
                             cmap=self.Cmap_type, vmax=0.5, vmin=-0.5)
        # ラベルの表示
        if self.Zeta1[:, 0].min() != self.Zeta1[:, 0].max():
            ...
            for i in range(self.Z1.shape[0]):
                ...
                for j in range(i):
                    if np.allclose(self.Z1[j, :], self.Z1[i, :]):
                        count += 1
                self.Map1.text(self.Z1[i, 0] - 0.01, self.Z1[i, 1] + epsilon * count, self.labels1[i], fontsize=12,
                               color='k')
        else:
            # epsilon = 0.02 * (self.Z1.max() - self.Z1.min())
            epsilon = 0.03
            for i in range(self.Z1.shape[0]):
                ...
                for j in range(i):
                    if np.allclose(self.Z1[j, :], self.Z1[i, :]):
                        count += 1
                if i // 2 != 0:
                    self.Map1.text(self.Z1[i, 1] - 0.01, self.Z1[i, 0] + epsilon * count, self.labels1[i], fontsize=12,
                                   color='k')
                else:
                    self.Map1.text(self.Z1[i, 1] - 0.01, self.Z1[i, 0] - epsilon * count, self.labels1[i], fontsize=12,
                                   color='k')
            ...
            for i in range(self.Z2.shape[0]):
                ...
                for j in range(i):
                    if np.allclose(self.Z2[j, :], self.Z2[i, :]):
                        count += 1
                self.Map2.text(self.Z2[i, 0] - 0.01, self.Z2[i, 1] + epsilon2 * count, self.labels2[i], fontsize=12,
                               color='k')
        else:
            # epsilon2 = 0.02 * (self.Z2.max() - self.Z2.min())
            epsilon2 = 0.03
            for i in range(self.Z2.shape[0]):
                ...
                for j in range(i):
                    if np.allclose(self.Z2[j, :], self.Z2[i, :]):
                        count += 1
                if i // 2 == 0:
                    self.Map2.text(self.Z2[i, 1] - 0.01, self.Z2[i, 0] + epsilon2 * count, self.labels2[i], fontsize=12,
                                   color='k')
                else:
                    self.Map2.text(self.Z2[i, 1] - 0.01, self.Z2[i, 0] - epsilon2 * count, self.labels2[i], fontsize=12,
                                   color='k')
