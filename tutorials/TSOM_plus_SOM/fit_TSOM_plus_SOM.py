...
for i in range(int(group_num/2)):
    ...
    group2 = X[int(xsamples*i / int(group_num/2)):int(xsamples*(i+1) / int(group_num/2)), int(ysamples / 2):int(ysamples), :]
...
#grad_normでteamを可視化
som_umatrix = Grad_Norm(X=tsom_plus_som.som.X,Z=tsom_plus_som.som.Z,sigma=0.1,labels=None,title_text="team_map",resolution=10)
