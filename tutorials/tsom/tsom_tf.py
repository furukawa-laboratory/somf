    ...
    def __init__(self, N1, N2, observed_dim, latent_dim, epochs, resolution, SIGMA_MAX, SIGMA_MIN, TAU, init='random'):
        ...
        with self.graph.as_default():
            ...
            if isinstance(self.init, str) and self.init == 'random':
                ...
                if isinstance(self.init[0], np.ndarray) and self.init[0].shape == (self.N1, self.latent_dim1):
                    with tf.name_scope('Z'):
                        self.Z1 = tf.Variable(initial_value=self.init[0].copy(), dtype=tf.float64)
                else:
                    raise ValueError("invalid inits[0]: {}".format(self.init))
                if isinstance(self.init[1], np.ndarray) and self.init[1].shape == (self.N2, self.latent_dim2):
                    with tf.name_scope('Z'):
                        self.Z2 = tf.Variable(initial_value=self.init[1].copy(), dtype=tf.float64)
                else:
